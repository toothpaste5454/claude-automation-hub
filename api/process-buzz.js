  ...
  try {
    ...
    if (geminiKey && topics.length > 0) {
      const THREAD_PATTERNS = [
        {
          name: 'hook_flow',
          structure: `【スレッド構成: フック→変化→メリット→使い方型】
スレッド全体が「問い→答え→深掘り」の流れでひとつの連続した話になるようにすること。

1. フック: 「これのすごいところは↓」「これで◯◯がこう変わる↓」「知らないと損する◯◯がついに↓」など次のツイートを見たくなるフレーズで必ず締めること。文中にソースURLを必ず含めること。改行は使わず1行で書くこと。
...
6. 締め: 自然にまとめられる場合のみ追加（無理に入れない）`,
        ...
        {
          name: 'story',
          structure: `【スレッド構成: ストーリー型】
「課題→転機→変化→展望」の流れで読者が自分ごと化できるストーリーを作ること。

1. 問題提起: 「〇〇で詰まってた話↓」「先週まで〇〇だったのが全部変わった↓」など読者が共感できる課題から始める。文中にソースURLを含めること。
2. 今まではこうだった→: 文頭に「今まではこうだった→」と入れ、現在の非効率・課題を具体的に描写。文末は「↓」
3. 転機になったのは→: 文頭に「転機になったのは→」と入れ、ツール・手法の登場を伝える。文末は「↓」
4. 使った結果→: 文頭に「使った結果→」と入れ、数字・具体例で変化を描写。文末は「↓」
5. 展望: 「これからは〇〇が当たり前になる」など一言まとめ
6. 締め: 自然にまとめられる場合のみ追加（無理に入れない）`,
        },
        {
          name: 'numbers',
          structure: `【スレッド構成: 数字・驚き型】
衝撃的なファクト・数字から始め、読者を引き込む構成にすること。

1. 衝撃ファクト: 「〇〇が〇%削減」「〇秒で〇〇ができる時代」など数字・統計から始める。文中にソースURLを含めること。文末は「↓」
2. なぜそうなるか→: 文頭に「なぜそうなるか→」と入れ、背景・仕組みを簡潔に説明。文末は「↓」
3. 具体的には→: 文頭に「具体的には→」と入れ、詳細・事例を列挙する。文末は「↓」
4. 自分でやるなら→: 文頭に「自分でやるなら→」と入れ、実践方法・ツール・始め方を伝える。文末は「↓」
5. 一言結論: 「つまり〇〇が変わる」など端的なまとめ
6. 締め: 自然にまとめられる場合のみ追加（無理に入れない）`,
        },
        {
          name: 'qa',
          structure: `【スレッド構成: Q&A型】
読者の疑問に答える形式で、知識欲を刺激する構成にすること。

1. 問い: 「〇〇って結局何が変わるの？↓」「〇〇使う人と使わない人、何が違うの？↓」など読者が抱きそうな疑問を立てる。文中にソースURLを含めること。
2. 一言で言うと→: 文頭に「一言で言うと→」と入れ、シンプルな答えを提示。文末は「↓」
3. もう少し詳しく言うと→: 文頭に「もう少し詳しく言うと→」と入れ、背景・仕組みを説明。文末は「↓」
4. 実際の例→: 文頭に「実際の例→」と入れ、具体的なユースケース・数字を示す。文末は「↓」
5. つまり→: 文頭に「つまり→」と入れ、一言で結論を伝える
6. 締め: 自然にまとめられる場合のみ追加（無理に入れない）`,
        },
      ]

      for (const threadTopic of topics) {
        const pattern = THREAD_PATTERNS[Math.floor(Math.random() * THREAD_PATTERNS.length)]
...
- 各ツイートは140文字以内（厳守）
- ハッシュタグは使わない
- 文体は「言い切り」にする（「広がる」「変わる」「できる」など。「広がります」などのですます調は使わない）
- 日本語で作成
