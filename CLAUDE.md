...
- **`check-github-usage.js`はGitHub Actions残り分数をTelegram通知するが、GitHub Billing APIの反映ラグにより実際の消費より少なめに出ることがある**（2026-07-03確認：自前APIが76%使用と返した同時刻に、GitHub公式Budgetアラートは90%使用だった）。枠逼迫の一次情報はGitHub公式のBudgetアラートメールを優先すること。
- **`check-github-usage`のトリガーはVercel CronではなくGitHub Actions（`.github/workflows/check-github-usage.yml`、毎日12:00 JST）**。Vercel Hobbyプランは1プロジェクトCron上限2つのため、3つ目として登録しても発火しない。新規Cronを追加する際は必ず`vercel.json`のcrons件数（2件以内）を確認すること。詳細は`tasks/lessons.md`の2026-07-03エントリを参照。
