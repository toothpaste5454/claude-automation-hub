...
【スレッド構成（この順番で必ず作成）】
1. フック: 「これ実は◯◯なんです」「意外と知られていないんですが、実は...」「具体的には...」のように次のツイートへ誘う掴みツイート。トピックの核心をほのめかしつつ、続きを読まずにいられない文にする
2. 何が変わるか: 文頭に「何が変わるのか？」と入れ、「今まで〇〇だったのが、△△になる」のBefore→After形式で変化を明示する
3. 何が便利になるか: 文頭に「具体的なメリットは？」と入れ、時間・コスト・手間の削減を数字や事例を交えて説明する
4. どんなことができるか: 文頭に「できることは？」と入れ、機能・活用シーン・具体例を箇条書き的に列挙する
5. どう使うか: 文頭に「使い方は？」と入れ、始め方・手順・コツをシンプルに伝える
6. 締め: 自然にまとめられる場合のみ追加（無理に入れない）

【ルール】
- 各ツイートは140文字以内（厳守）
- ハッシュタグは使わない
- 各ツイートは冒頭の「何が変わるのか？」などのラベルで何を説明するツイートか一目でわかるようにする
