...
### ステップ2: Xバズトレンドを調査する（WebSearch）

まずWebSearchツールで以下のクエリを実行し、今日Xで実際にバズっている投稿・話題を調査する：

- `site:x.com Claude OR ChatGPT OR Gemini OR AI 最新 2026`
- `X Twitter バズ AI Claude 話題 ${TODAY}`

調査結果から、**今日実際にXで拡散している話題を2件**ピックアップする（`source: "x_buzz"`）。
以下の基準で選ぶ：
- いいね・RT・引用が多い投稿のトピック
- 実際の体験談・Before/After・驚きのある内容
- AI/テクノロジー系（Claude・ChatGPT・Gemini・新ツール・新機能など）

WebSearchで有力な情報が得られなかった場合は、Claudeの知識ベースから「今日話題になっていそうなXバズ的トピック」を2件補完する。

### ステップ3: AI/テクノロジートレンドを4件選定

リモートセッションではGoogle News RSSがネットワーク制限でブロックされるため、
**Claudeの知識ベースから直接4件選定する**（RSS取得は試みなくてよい）。

- 国内トレンド 2件（`source: "domestic"`）
- 海外トレンド 2件（`source: "international"`）
...
4. **プレスリリース・発表記事は4件中最大1件まで**（「〜が発表」だけで使い方が書けないものは選ばない）
5. 数字（スター数、削減率、処理時間、価格など）が1つ以上書けるトピックを優先

ステップ2のXバズ2件と合わせて、合計6件になるようにすること。
...
- `genre`: "AI/テクノロジー"
- `source`: "domestic" / "international" / "x_buzz"
  ...
  "topics": [
    {
      ...
      "url": "https://...",
      "genre": "AI/テクノロジー",
      "source": "domestic または international または x_buzz"
    }
  ],
  "posts": [
    {
