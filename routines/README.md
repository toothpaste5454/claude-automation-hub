...
| トリガーID | 名前 | JST | プロンプト | リポジトリ |
|---|---|---|---|---|
| `trig_017LyrzBCaqNUiEcn6XA4Hft` | X-buzz投稿生成 v4 | 8:00 | `task2_xbuzz.md` | claude-automation-hub |
| `trig_01Cp4th4FBhYNb7f4n5ysw1k` | Livex物件チェック v2 | 9:00 | `task1_livex.md` | claude-automation-hub |
| `trig_0149jRqrxoZgGvx2nXDyspha` | Livex物件チェック v2 | 15:00 | `task1_livex.md` | claude-automation-hub |
| `trig_012GodStUcHyaXgwhZk1g1Yp` | Livex物件チェック v2 | 21:00 | `task1_livex.md` | claude-automation-hub |

**v2方式**: 全手順・設定値をプロンプトに直接埋め込み。CLAUDE.md参照やget-config API呼び出しは不要。
...
| git push 403 | リモートセッションのgit proxyにwrite権限なし | プロンプトでgit remote set-url にtokenを埋め込む |
| process-buzz 404 | 日付ミスマッチ（JST vs セッション日時） | `?date=YYYY-MM-DD` で手動指定して実行 |
